...
export class UserConsentRepo {
    ...
    static findUserConsentByPlatformId = async (container: DependencyContainer, platformId: string):
    Promise<UserConsentDto | null> => {
        try {
    ...
    static updateUserConsent = async (container, platformId: string, consentGiven: string):
    Promise<UserConsentDto | null> => {
        try {
            const entityManager = await RepositoryHelper.resolveEntityManager(container);
            const userConsentRepository = entityManager.getRepository(UserConsent);
