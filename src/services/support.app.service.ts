...
export const getPatientsByPhoneNumberservice = async (phoneNumber, patientNumber = null) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            let apiUrl = `${ReanBackendBaseUrl}/patient/get-by-phone?phoneNumber=${encodeURIComponent(phoneNumber)}`
            let response = await needle("get", apiUrl, options)
            ...
            if (response.body.data.Patients.length > 1) {
                ...
                if (patientNumber > 0) {
                    let selectedPatient = response.body.data.Patients[patientNumber - 1]
...
export const getMedicationInfoservice = async (patientUserId, accessToken) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            let apiUrl = `${ReanBackendBaseUrl}/medication?patientUserId=${patientUserId}`
            ...
            let data = { "fulfillmentMessages": [{ "text": { "text": [dffMessage] } }] }
