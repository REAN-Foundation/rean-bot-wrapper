...
export class GetPatientInfoService{
    ...
    getPatientsByPhoneNumberservice = async (eventObj) => {
        try {
            ...
            if (response.body.Data.Patients.Items.length === 0) {
                return ({ sendDff: true, message: { "fulfillmentMessages": [{ "text": { "text": ['No patient found for given phone number'] } }]  } });
            ...
            return ({ sendDff: sendDff, message: data });
        }
        catch (error) {
