...
export class GenerateCertificateService {
    ...
    async generateCertificateInformVolunteer(body) {
        try {
            ...
            const donationDate = await TimeHelper.formatDateToLocal_YYYY_MM_DD(new Date(body.DonationRecord.DonorAcceptedDate));
            ...
            const buttons = await sendApiButtonService(["Yes", "Generate_Certificate_Yes", "No", "Generate_Certificate_No"]);
