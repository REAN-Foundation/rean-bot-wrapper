...
export class InitiateDeleteReminderService {
    ...
    async initiateDelete (eventObj) {
        try {
            ...
            let message = null;
            const getreminderurl = `reminders/search?userId=${patientIDArray.patientUserId}`;
    ...
    async getReminderDetails (eventObj) {
        try {
            ...
            const patientIDArray = await this.registration.getPatientUserId(channelName,
                sessionId, userName);
            let message = null;
            const getreminderurl = `reminders/search?userId=${patientIDArray.patientUserId}&name=${messageBodyList[0]}`;
            const responseBody = await this.needleService.needleRequestForREAN("get", getreminderurl);
            const listOfReminders = responseBody.Data.Reminders.Items;
            if (listOfReminders.length > 0) {
                const buttonArray = [];
    ...
    async deleteRemider (eventObj) {
        try {
            ...
            const patientIDArray = await this.registration.getPatientUserId(channelName,
                sessionId, userName);
            let message = null;
            const getreminderurl = `reminders/search?userId=${patientIDArray.patientUserId}&name=${cache.reminderName}&whenTime=${whenTime}&reminderType=${reminderType}`;
            const responseBody = await this.needleService.needleRequestForREAN("get", getreminderurl);
            const listOfReminders = responseBody.Data.Reminders.Items;
            if (listOfReminders.length > 0) {
                for (const reminder of listOfReminders) {
