...
export class databackup{
    ...
    async getKeys(clientName){
        ...
        const folderName = process.env[clientName + "_" + "FORM_NAME"];
        return [URL,assetIDs,myToken,folderName];
    }
    ...
    async getData(URL,assetID,myToken){
      ...
      const NoOfDataPoints = response_data.length;
    ...
    async main(clientName){
        const [URL,assetIDs,myToken,folderName] = await this.getKeys(clientName);
